...
public class OWLAPI2OrarConverter {
	...
	private void obtainSignature() {
		this.internalOntology.addIndividualsToSignature(
				indexer.getIndexesOfOWLIndividuals(owlOntology.getIndividualsInSignature(true)));
		// logger.info("***DEBUG*** all
		// individuals:"+owlOntology.getIndividualsInSignature(true));
		...
		for (OWLObjectPropertyAssertionAxiom assertion : owlOntology.getAxioms(AxiomType.OBJECT_PROPERTY_ASSERTION,
			...
			if (property instanceof OWLObjectProperty) {
				if (subject instanceof OWLNamedIndividual && object instanceof OWLNamedIndividual) {
					this.internalOntology.addRoleAssertion(subjectIndex, property.asOWLObjectProperty(), objectIndex);
				}
			}

			if (property instanceof OWLObjectInverseOf) {
				if (subject instanceof OWLNamedIndividual && object instanceof OWLNamedIndividual) {
					this.internalOntology.addRoleAssertion(objectIndex, property.getNamedProperty(), subjectIndex);
				}
			}
